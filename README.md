...
Detecting overflow may be harder than it looks. To make it happen, you have to store the processed value into a long and count the number of digits proceeded by your function. If this number becomes greater than the number of digits included in the max long (19), then there is an overflow. You also have to check that the processed value is still greater than 0. The purpose of doing this is to detect an overflow when the value is greater than the max long but still has the same number of digits. You may be wondering what's the point to count the digits if you can detect an overflow by checking that the number is still greater than 0: it's because huge numbers may lead to several overflows which can't be reliably detected with simple comparisons.
